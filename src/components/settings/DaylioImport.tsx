...
export default function DaylioImport({ groups, existingDates }: Props) {
  ...
  function handleFileChange(e: { target: HTMLInputElement }) {
    ...
    reader.onload = (ev) => {
      ...
      setStep({
        type: 'parsed',
        summary: { rows, parseErrors: errors, willSkip, unmatchedActivities, unmatchedScales, matchedActivities, matchedScales, dateRange },
      });
    };
  ...
  async function runImport(summary: ImportSummary) {
    ...
    try {
      ...
      const result = await apiService.importDaylio(payload);
      setStep({ type: 'done', imported: result.imported, skipped: result.skipped });
    } catch (err) {
      setStep({ type: 'error', message: err instanceof Error ? err.message : 'Import failed.' });
    }
  }
  ...
  if (step.type === 'parsed') {
    const { summary } = step;
    const needsMapping = summary.unmatchedActivities.length > 0 || summary.unmatchedScales.length > 0;
              ...
              {summary.unmatchedActivities.map((act) => {
                ...
                return (
                  <div key={act} className="flex flex-wrap items-center gap-2 py-2 border-t border-[var(--border)]">
                    <span className="text-sm text-[var(--text)] min-w-[120px] font-medium">{act}</span>
                    <select
                ...
                return (
                  <div key={scale.name} className="flex flex-wrap items-center gap-2 py-2 border-t border-[var(--border)]">
                    ...
                    <select
                      ...
                      className="text-sm border border-[var(--border)] rounded bg-[var(--bg)] text-[var(--text)] px-2 py-1 flex-1 min-w-[200px]"
                    >
                      <option value="create">Create new slider "{scale.name}"</option>
                      {sliderGroups.map((g) => (
  ...
  if (step.type === 'importing') {
    return (
      <p className="text-sm text-[var(--text-muted)]" role="status">Importing — please wait…</p>
    );
  }
