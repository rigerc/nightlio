...
const MoodIconCustomizer = () => {
  ...
  const [openPicker, setOpenPicker] = useState(null); // mood value that has picker open
  const [saving, setSaving] = useState(false);

  const handleIconChange = async (moodValue, iconName) => {
    setSaving(true);
    try {
              ...
              {isOpen && (
                <div className="ml-12 mb-2">
                  <IconPicker
                    ...
                    icons={MOOD_ICONS}
                    onChange={name => handleIconChange(mood.value, name)}
                  />
                </div>
              )}
            </div>
