    ...
    <div className="bg-[var(--surface)] rounded-2xl p-6 border border-[var(--border)] shadow-md mt-4">
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          ...
          className="bg-transparent border-none cursor-pointer text-[var(--text-muted)] p-1 hover:text-[var(--text)] transition-colors"
        >
          <X size={20} />
        </button>
      </div>
              ...
              <div key={group.id} className="p-4 bg-[var(--bg)] rounded-lg border border-[var(--border)]">
                ...
                <div className="flex flex-wrap gap-1">
                  {group.options.map(option => (
                    <span
                      key={option.id}
                      className="px-2 py-0.5 bg-[var(--surface)] border border-[var(--border)] rounded-xl text-xs text-[var(--text-muted)]"
                    >
                      {option.name}
                    </span>
                  ))}
