...
const ICONS: Record<string, LucideIcon> = { Zap, Flame, Crown, BarChart3, Target };

const RARITY_STYLES: Record<Rarity, { background: string; border: string; badge: string }> = {
...
const AchievementCard = ({ achievement, isUnlocked, progressValue, progressMax, onClick }: AchievementCardProps) => {
  ...
  return (
    <div
      ...
      style={{ background: style.background, border: `1px solid ${style.border}`, borderRadius: 16, padding: '1.25rem', cursor: 'pointer', display: 'flex', flexDirection: 'column', gap: '0.75rem', opacity: isUnlocked ? 1 : 0.65, transition: 'box-shadow 0.18s ease, opacity 0.18s ease', outline: 'none' }}
      onMouseEnter={(e) => { e.currentTarget.style.boxShadow = 'var(--shadow-md)'; }}
      onMouseLeave={(e) => { e.currentTarget.style.boxShadow = 'none'; }}
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
        <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 40, height: 40, borderRadius: '50%', background: `${style.badge}22`, color: style.badge, flexShrink: 0 }}>
          <Icon size={22} strokeWidth={1.8} />
        </span>
        <div style={{ minWidth: 0 }}>
          <div style={{ fontWeight: 600, color: 'var(--text)', fontSize: '0.95rem' }}>{achievement.name}</div>
          <span style={{ fontSize: '0.72rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', color: style.badge }}>{achievement.rarity}</span>
        </div>
        {isUnlocked && (
          <span style={{ marginLeft: 'auto', fontSize: '0.75rem', fontWeight: 600, background: '#22c55e22', color: '#16a34a', borderRadius: 999, padding: '2px 10px', border: '1px solid #22c55e44', flexShrink: 0 }}>Unlocked</span>
        )}
      </div>
      <p style={{ margin: 0, fontSize: '0.875rem', color: 'var(--text-muted)', lineHeight: 1.5 }}>{achievement.description}</p>
      {!isUnlocked && progressMax > 0 && <ProgressBar value={progressValue} max={progressMax} />}
    </div>
  );
};

interface ProgressEntry { current: number; max: number; }
...
const AchievementsView = () => {
  ...
  useEffect(() => {
    const loadAchievements = async () => {
      try {
        ...
        setProgress((prog as Record<string, ProgressEntry>) || {});
      } catch (err) {
  ...
  if (loading) return <div style={{ marginTop: '2rem', textAlign: 'center', color: 'var(--text-muted)' }}>Loading achievements…</div>;
  if (error) return <div style={{ marginTop: '2rem', textAlign: 'center', color: 'var(--accent-600)' }}>{error}</div>;
  ...
  return (
    <div style={{ marginTop: '1.5rem' }}>
        ...
        {getAllAchievements().map((achievement) => {
          ...
          return (
            <AchievementCard
              ...
              achievement={unlocked ? { ...achievement, ...unlocked, rarity: unlocked.rarity as Rarity } : achievement}
              isUnlocked={isUnlocked}
              progressValue={isUnlocked ? 0 : (p ? p.current : 0)}
      ...
      <Modal open={!!active} onClose={() => setActive(null)} title={active?.name ?? 'Achievement'}>
        ...
        {active && !achievements.find(a => a.achievement_type === active.achievement_type) && (() => {
