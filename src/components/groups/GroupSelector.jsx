...
const tintBg = (hex) => hex ? hex + '18' : null;
const tintBorder = (hex) => hex ? hex + '50' : null;
    ...
    <div className="mb-8">
      {groups.map(group => {
        ...
        return (
          ...
          >
            <h3
              className="m-0 mb-3 text-base font-medium tracking-wide flex items-center gap-2"
              style={{ color: headerColor }}
            >
              {GroupIconComp && (
                <GroupIconComp size={16} strokeWidth={1.5} style={{ color: headerColor }} />
              )}
              {group.name}
            </h3>
            <div className="flex flex-wrap gap-2">
              {group.options.map(option => {
                ...
                return (
                  ...
                  >
                    {OptionIconComp && (
                      <OptionIconComp size={13} strokeWidth={1.5} className="shrink-0" />
                    )}
                    {option.name}
                  </button>
                );
