...
const MonthCalendar = ({ year, month, dates }: MonthCalendarProps) => {
        ...
        {cells.map((d, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              color: isHit ? '#fff' : 'var(--text)', display: 'grid', placeItems: 'center', fontSize: 12,
            }}>
              {d || ''}
            </div>
          );
...
const GoalStatsCalendar = ({ goalId }: GoalStatsCalendarProps) => {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        if (!mounted) return;
        setDates(new Set((rows || []).map((r: { date: string }) => r.date)));
      } catch {
